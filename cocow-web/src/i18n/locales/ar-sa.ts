import type { Translations } from "./en-us";

export const arSA: Translations = {
  site: {
    title: "Cocow",
  },
  nav: {
    subtitle: "مركز التنقل الشخصي الخاص بك",
    searchPlaceholder: "البحث عن المواقع...",
    noResults: "لم يتم العثور على مواقع",
    noWebsites: "لا توجد مواقع متاحة",
    loading: "جارٍ التحميل...",
  },
  language: {
    name: "العربية",
    switch: "تبديل اللغة",
  },
  categories: {
    all: "الكل",
  },
  theme: {
    coconut: "أبيض جوز الهند",
    ocean: "أزرق المحيط",
    forest: "أخضر الغابة",
    sunset: "برتقالي الغروب",
    midnight: "بنفسجي منتصف الليل",
  },
  searchEngine: {
    prefix: "البحث باستخدام ",
    suffix: "...",
  },
  user: {
    account: "الحساب",
    profileSettings: "إعدادات الملف الشخصي",
    signOut: "تسجيل الخروج",
    signIn: "تسجيل الدخول",
  },
  userWebsites: {
    title: "مواقعي",
    add: "إضافة موقع",
    remove: "إزالة",
    pin: "تثبيت",
    unpin: "إلغاء التثبيت",
    moveUp: "تحريك لأعلى",
    moveDown: "تحريك لأسفل",
    searchPlaceholder: "ابحث عن مواقع للإضافة...",
    noResults: "لم يتم العثور على مواقع مطابقة",
    empty: "لا توجد مواقع في قائمتك بعد",
  },
  admin: {
    title: "إدارة المستخدمين",
    back: "رجوع",
    totalUsers: "إجمالي المستخدمين",
    noUsers: "لم يتم العثور على مستخدمين",
    sub: "معرف المستخدم",
    role: "الدور",
    createdAt: "تاريخ الإنشاء",
    actions: "الإجراءات",
    promote: "ترقية إلى مشرف",
    demote: "خفض إلى مستخدم",
    updating: "جارٍ التحديث...",
    roleUpdated: "تم تحديث الدور بنجاح",
    prev: "السابق",
    next: "التالي",
    pageInfo: "{page} / {total}",
  },
  common: {
    cancel: "إلغاء",
  },
};
